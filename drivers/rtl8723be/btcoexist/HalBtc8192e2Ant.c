...
const char *const GLbt_infoSrc8192e2Ant[]={
	"BT Info[wifi fw]",
	"BT Info[bt rsp]",
	"BT Info[bt auto report]",
};

u32	GLCoexVerDate8192e2Ant=20130902;
u32	GLCoexVer8192e2Ant=0x34;
	...
	)
{
	...
	pCoexDm->curra_maskType = ra_maskType;
	disra_mask = halbtc8192e2ant_Decidera_mask(pBtCoexist, pCoexDm->curSsType, ra_maskType);
	halbtc8192e2ant_Updatera_mask(pBtCoexist, bForceExec, disra_mask);
	...
	)
{
	...
	if(bAgcTableEn)
	{
		...
		pBtCoexist->btc_write_4byte(pBtCoexist, 0xc78, 0x0a1A0001);
		pBtCoexist->btc_write_4byte(pBtCoexist, 0xc78, 0x091B0001);
		pBtCoexist->btc_write_4byte(pBtCoexist, 0xc78, 0x081C0001);
		pBtCoexist->btc_write_4byte(pBtCoexist, 0xc78, 0x071D0001);
		pBtCoexist->btc_write_4byte(pBtCoexist, 0xc78, 0x061E0001);
		pBtCoexist->btc_write_4byte(pBtCoexist, 0xc78, 0x051F0001);
	}
	else
	{
	...
	)
{
	switch(type)
	{
		...
		case 2:
			...
			break;
		case 3:
			halbtc8192e2ant_CoexTable(pBtCoexist, bForceExec, 0xdfffdfff, 0x5fdb5fdb, 0xffffff, 0x3);
			break;
		case 4:
	...
	)
{
	...
	else
	{
		...
		switch(type)
		{
			default:
			case 0:
				//halbtc8192e2ant_SetFwPstdma(pBtCoexist, 0x0, 0x0, 0x0, 0x40, 0x0);
				halbtc8192e2ant_SetFwPstdma(pBtCoexist, 0x8, 0x0, 0x0, 0x0, 0x0);
				pBtCoexist->btc_write_1byte(pBtCoexist, 0x92c, 0x4);
				break;
			case 1:
				//halbtc8192e2ant_SetFwPstdma(pBtCoexist, 0x0, 0x0, 0x0, 0x48, 0x0);
	...
	)
{
	...
	disra_mask = halbtc8192e2ant_Decidera_mask(pBtCoexist, ssType, pCoexDm->curra_maskType);
	halbtc8192e2ant_Updatera_mask(pBtCoexist, FORCE_EXEC, disra_mask);
...
{
	static int		up,dn,m,n,WaitCount;
	int			result;   //0: no change, +1: increase WiFi duration, -1: decrease WiFi duration
	u8			retryCount=0;
...
{
	u8	wifiRssiState, btRssiState=BTC_RSSI_STATE_STAY_LOW;
	u32	wifiBw;
...
{
	u8	wifiRssiState, btRssiState=BTC_RSSI_STATE_HIGH;	
	u32	wifiBw;
	...
	)
{
	...
	u8				bt_infoOriginal=0, btRetryCnt=0;
	u8				algorithm=0;
	...
	)
{
	...
	pBtCoexist->btc_write_1byte(pBtCoexist, 0x92c, 0x20);
	if(pBtCoexist->chip_interface == BTC_INTF_USB)
	...
	)
{
	...
	CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = [%s/ %d/ %d] ", "BT [status/ rssi/ retryCnt]", \
		((pBtCoexist->bt_info.bt_disabled)? ("disabled"):	((pCoexSta->bC2hBtInquiryPage)?("inquiry/page scan"):((BT_8192E_2ANT_BT_STATUS_NON_CONNECTED_IDLE == pCoexDm->btStatus)? "non-connected idle":
	...
	for(i=0; i<BT_INFO_SRC_8192E_2ANT_MAX; i++)
	{
		...
		{				
			CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = %02x %02x %02x %02x %02x %02x %02x(%d)", GLbt_infoSrc8192e2Ant[i], \
...
void
EXhalbtc8192e2ant_bt_infoNotify(
	IN	struct btc_coexist *		pBtCoexist,
	IN	u8 *			tmpBuf,
	IN	u8			length
	)
{
	...
	if(!(bt_info&BT_INFO_8192E_2ANT_B_CONNECTION))
	{
		...
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], bt_infoNotify(), BT Non-Connected idle!!!\n"));
	}
	else if(bt_info == BT_INFO_8192E_2ANT_B_CONNECTION)	// connection exists but no busy
	{
		pCoexDm->btStatus = BT_8192E_2ANT_BT_STATUS_CONNECTED_IDLE;
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], bt_infoNotify(), BT Connected-idle!!!\n"));
	}
	else if((bt_info&BT_INFO_8192E_2ANT_B_SCO_ESCO) ||
		(bt_info&BT_INFO_8192E_2ANT_B_SCO_BUSY))
	{
		pCoexDm->btStatus = BT_8192E_2ANT_BT_STATUS_SCO_BUSY;
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], bt_infoNotify(), BT SCO busy!!!\n"));
	}
	else if(bt_info&BT_INFO_8192E_2ANT_B_ACL_BUSY)
	{
		pCoexDm->btStatus = BT_8192E_2ANT_BT_STATUS_ACL_BUSY;
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], bt_infoNotify(), BT ACL busy!!!\n"));
	}
	else
	{
		pCoexDm->btStatus = BT_8192E_2ANT_BT_STATUS_MAX;
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], bt_infoNotify(), BT Non-Defined state!!!\n"));
	}
...
#if(BT_AUTO_REPORT_ONLY_8192E_2ANT == 0)
	halbtc8192e2ant_Querybt_info(pBtCoexist);
	halbtc8192e2ant_MonitorBtCtr(pBtCoexist);
