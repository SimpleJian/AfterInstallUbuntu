...
static void _rtl8723be_set_fw_clock_on(struct ieee80211_hw *hw,
...
static void _rtl8723be_set_fw_clock_off(struct ieee80211_hw *hw,
	u8 rpwm_val)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
...
void rtl8723be_set_hw_reg(struct ieee80211_hw *hw, u8 variable, u8 *val)
{
	...
	case HW_VAR_KEEP_ALIVE: {
			...
			array[0] = 0xff;
			array[1] = *((u8 *)val);
			rtl8723be_fill_h2c_cmd(hw, H2C_88E_KEEP_ALIVE_CTRL, 2, array);
		}
	default:
...
static bool _rtl8723be_init_mac(struct ieee80211_hw *hw)
{
	...
	if (!rtl_hal_pwrseqcmdparsing(rtlpriv, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
		...
		RT_TRACE(COMP_INIT,DBG_LOUD,("init MAC Fail as power on failure\n"));
		return false;
	}
...
int rtl8723be_hw_init(struct ieee80211_hw *hw)
{
	...
	err = rtl8723be_download_fw(hw, false);
	if (err) {
		RT_TRACE(COMP_ERR, DBG_WARNING,
	...
	//		rtl8723be_phy_iq_calibrate(hw, true);
		} else {
...
static enum version_8723be _rtl8723be_read_chip_version(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_phy *rtlphy = &(rtlpriv->phy);
	enum version_8723be version = VERSION_UNKNOWN;
	u8	count = 0;
	u8	value8;
	u32 value32;
	...
	if ((value32 & (CHIP_8723B)) != CHIP_8723B){
		RT_TRACE(COMP_INIT, DBG_LOUD, ("unkown chip version\n"));
	}else{
		version = (enum version_8723be) VERSION_TEST_CHIP_1T1R_8723B;
	}
	...
	u8 *hwinfo)
{
	...
	RT_TRACE(COMP_INIT, DBG_LOUD, ("hal_ReadPowerValueFromPROM8723BE(): PROMContent[0x%x]=0x%x\n", (eeAddr+1), hwinfo[eeAddr+1]));
						 ...
						 u8 *hwinfo)
{
	...
	_rtl8723be_read_power_value_fromprom(hw, &pwrinfo24g, &pwrinfo5g, autoload_fail, hwinfo);
...
static void _rtl8723be_read_adapter_info(struct ieee80211_hw *hw, bool b_pseudo_test )
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_efuse *rtlefuse = rtl_efuse(rtl_priv(hw));
	struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
	...
	_rtl8723be_read_txpower_info_from_hwpg(hw,
		...
		struct ieee80211_sta *sta, u8 rssi_level)
{
	...
	*(u32 *) & rate_mask = EF4BYTE((ratr_bitmap & 0x0fffffff) |
				       (ratr_index << 28));
	rate_mask[0] = macid;
	rate_mask[1] = _rtl8723be_mrate_idx_to_arfr_id(hw, ratr_index) | (b_shortgi ? 0x80 : 0x00);
		...
		struct ieee80211_sta *sta, u8 rssi_level)
{
	...
	if (rtlpriv->dm.b_useramask)
		...
		/*RT_TRACE(COMP_RATR,DBG_LOUD,("rtl8723be_update_hal_rate_tbl(): Error! 8723BE FW RA Only"));*/
...
void rtl8723be_allow_all_destaddr(struct ieee80211_hw *hw,
