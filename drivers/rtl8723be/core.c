...
static int rtl_op_config(struct ieee80211_hw *hw, u32 changed)
{
	...
	if (changed & IEEE80211_CONF_CHANGE_CHANNEL) {
		...
		enum nl80211_channel_type channel_type = cfg80211_get_chandef_type(&(hw->conf.chandef));  /* added by binxia */ 
#else
			     ...
			     struct ieee80211_bss_conf *bss_conf, u32 changed)
{
	...
	if (changed & BSS_CHANGED_ASSOC) {
		...
		if (rtlpriv->cfg->ops->get_btc_status()){
			rtlpriv->btcoexist.btc_ops->btc_mediastatus_notify(rtlpriv, ppsc->report_linked);
		}
	}
			       ...
			       )
{
	...
	case IEEE80211_AMPDU_TX_STOP_CONT:		/*added by binxia*/
