...
u8 halbtc_get_wifi_central_chnl(struct btc_coexist *btcoexist)
{
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("halbtc_get_wifi_central_chnl:%d\n",chnl));
	return chnl;
}
...
bool halbtc_get(void *void_btcoexist, u8	get_type, void *out_buf)
{
	...
	case BTC_GET_BL_WIFI_4_WAY_PROGRESS:	/*TODO*/
		/*if(mac->opmode == NL80211_IFTYPE_STATION) &&
			(rtlpriv->sec.pairwise_enc_algorithm == dot11AuthAlgrthm_8021X ||
			rtlpriv->securitypriv.dot11AuthAlgrthm == dot11AuthAlgrthm_WAPI ||
			check_fwstate(&rtlpriv->mlmepriv, WIFI_UNDER_WPS) == _true) &&
			(rtlpriv->securitypriv.bStaInstallPairwiseKey == _false)){
			*bool_tmp = true;
		}
		else*/
			*bool_tmp = false;
...
/*
			...
			*bool_tmp = wdev_to_priv(rtlpriv->rtw_wdev)->bandroid_dhcp;
#else
			*bool_tmp = &rtlpriv->mlmepriv.android_bdhcp;
...
/*		if (mgnt_link_status_query(rtlpriv->mac80211.hw) == RT_MEDIA_CONNECT) {
			updatera_mask();
		}*/
		break;
...
void halbtc_bitmask_write_1byte(void *bt_context, u32 reg_addr, u8 bit_mask, u8 data)
{
	struct btc_coexist *btcoexist = (struct btc_coexist *)bt_context;
	struct rtl_priv *rtlpriv = btcoexist->adapter;
	...
	if (bit_mask != MASKDWORD) {/*if not "double word" write*/
		...
		bit_shift = i;
		data = ((original_value) & (~bit_mask)) |( ((data << bit_shift)) & bit_mask);
	}
	rtl_write_byte(rtlpriv, reg_addr, data);
}
...
void halbtc_set_rfreg(void *bt_context, u8 rf_path, u32 reg_addr, u32 bit_mask, u32 data)
{
	struct btc_coexist *btcoexist = (struct btc_coexist *)bt_context;
	struct rtl_priv *rtlpriv = btcoexist->adapter;
...
void halbtc_fill_h2c_cmd(void *bt_context, u8 element_id, u32 cmd_len, u8 *cmd_buf)
{
	struct btc_coexist *btcoexist = (struct btc_coexist *)bt_context;
	struct rtl_priv *rtlpriv = btcoexist->adapter;
	
	rtlpriv->cfg->ops->fill_h2c_cmd(rtlpriv->mac80211.hw, element_id, cmd_len, cmd_buf);
}
...
bool exhalbtc_initlize_variables(struct rtl_priv *adapter)
{
	...
	btcoexist->stack_info.bProfileNotified = false;
...
void exhalbtc_init_hw_config(struct btc_coexist *btcoexist)
{
	...
	if (rtlhal->hw_type == HARDWARE_TYPE_RTL8723BE) {
		...
		EXhalbtc8192e2ant_InitHwConfig(btcoexist);
	}
...
void exhalbtc_init_coex_dm(struct btc_coexist *btcoexist)
{
	...
	if (rtlhal->hw_type == HARDWARE_TYPE_RTL8723BE) {
		...
		EXhalbtc8192e2ant_InitCoexDm(btcoexist);
	}
...
void exhalbtc_ips_notify(struct btc_coexist *btcoexist, u8 type)
{
	...
	if (rtlhal->hw_type == HARDWARE_TYPE_RTL8723BE) {
		...
		EXhalbtc8192e2ant_IpsNotify(btcoexist, ips_type);
	}
...
void exhalbtc_lps_notify(struct btc_coexist *btcoexist, u8 type)
{
	...
	if (rtlhal->hw_type == HARDWARE_TYPE_RTL8723BE) {
		...
		EXhalbtc8192e2ant_LpsNotify(btcoexist, lps_type);
	}
}
...
void exhalbtc_scan_notify(struct btc_coexist *btcoexist, u8 type)
{
	...
	if (rtlhal->hw_type == HARDWARE_TYPE_RTL8723BE) {
		...
		EXhalbtc8192e2ant_ScanNotify(btcoexist, scan_type);
	}
...
void exhalbtc_connect_notify(struct btc_coexist *btcoexist, u8 action)
{
	...
	if (rtlhal->hw_type == HARDWARE_TYPE_RTL8723BE) {
		...
		EXhalbtc8192e2ant_ConnectNotify(btcoexist, asso_type);
	}
...
void exhalbtc_mediastatus_notify(struct btc_coexist *btcoexist, enum rt_media_status media_status)
{
	...
	if (rtlhal->hw_type == HARDWARE_TYPE_RTL8723BE) {
		...
		EXhalbtc8192e2ant_MediaStatusNotify(btcoexist, status);
	}
...
void exhalbtc_special_packet_notify(struct btc_coexist *btcoexist, u8 pkt_type)
{
	...
	if (btcoexist->board_info.btdm_ant_num == 2)
		ex_halbtc8723b2ant_special_packet_notify(btcoexist, packet_type);
	else if(btcoexist->board_info.btdm_ant_num == 1)
		ex_halbtc8723b1ant_special_packet_notify(btcoexist, packet_type);
...
void exhalbtc_bt_info_notify(struct btc_coexist *btcoexist, u8 *tmp_buf, u8 length)
{
	...
	//	EXhalbtc8192e2ant_bt_infoNotify(btcoexist, tmp_buf, length);
	}
}
...
void exhalbtc_stack_operation_notify(struct btc_coexist *btcoexist, u8 type)
{
	...
	if (btcoexist->board_info.btdm_ant_num == 2)
		ex_halbtc8723b2ant_stack_operation_notify(btcoexist, stack_op_type);
	else if(btcoexist->board_info.btdm_ant_num == 1)
		ex_halbtc8723b1ant_stack_operation_notify(btcoexist, stack_op_type);
...
void exhalbtc_halt_notify(struct btc_coexist *btcoexist)
{
	...
	if (rtlhal->hw_type == HARDWARE_TYPE_RTL8723BE) {
		...
		EXhalbtc8192e2ant_HaltNotify(btcoexist);
	}
}
...
void exhalbtc_periodical(struct btc_coexist *btcoexist)
{
	...
	if (rtlhal->hw_type == HARDWARE_TYPE_RTL8723BE) {
		...
		EXhalbtc8192e2ant_Periodical(btcoexist);
	}
...
void exhalbtc_update_min_bt_rssi(char bt_rssi)
{
	...
	btcoexist->stack_info.minBtRssi = bt_rssi;
}
...
void exhalbtc_set_hci_version(u16 hci_version)
{
	...
	btcoexist->stack_info.hciVersion = hci_version;
}
