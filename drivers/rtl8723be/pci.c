	...
	u8 *entry, int rxring_idx, int desc_idx)
{
	...
	*((dma_addr_t *) skb->cb) = pci_map_single(rtlpci->pdev,
	...
	if (pci_dma_mapping_error(rtlpci->pdev, bufferaddress))
		return 0;
	if (rtlpriv->use_new_trx_flow) {
		rtlpriv->cfg->ops->set_desc(hw, (u8 *) entry, false, HW_DESC_RX_PREPARE,
				    (u8 *) & bufferaddress);
	} else {	
		rtlpriv->cfg->ops->set_desc(hw, (u8 *) entry, false, HW_DESC_RXBUFF_ADDR,
				    (u8 *) & bufferaddress);
		rtlpriv->cfg->ops->set_desc(hw, (u8 *) entry, false, HW_DESC_RXPKT_LEN,
				    (u8 *) & rtlpci->rxbuffersize);
		rtlpriv->cfg->ops->set_desc(hw, (u8 *) entry, false, HW_DESC_RXOWN,
				    (u8 *) & tmp_one);
	}
...
static void _rtl_pci_rx_interrupt(struct ieee80211_hw *hw)
{
	...
	while (count--) {
		...
		struct rtl_rx_desc *pdesc;/*if use new trx flow, it means wifi info */
		/*rx pkt */
		struct sk_buff *skb = rtlpci->rx_ring[rxring_idx].rx_buf[
				rtlpci->rx_ring[rxring_idx].idx];
		
		if (rtlpriv->use_new_trx_flow) {
			rx_remained_cnt = rtlpriv->cfg->ops->rx_desc_buff_remained_cnt(hw, hw_queue);
			...
			pdesc = (struct rtl_rx_desc *)skb->data;/*means rx wifi info*/
		} else {
			pdesc = &rtlpci->rx_ring[rxring_idx].desc[
				rtlpci->rx_ring[rxring_idx].idx];/* rx descriptor */
		...
		if (rtlpriv->use_new_trx_flow)
			rtlpriv->cfg->ops->rx_check_dma_ok(hw, (u8 *)buffer_desc, hw_queue);

		pci_unmap_single(rtlpci->pdev, *((dma_addr_t *) skb->cb),
			 rtlpci->rxbuffersize, PCI_DMA_FROMDEVICE);
			
		len = rtlpriv->cfg->ops->get_desc((u8 *)pdesc, false, HW_DESC_RXPKT_LEN);
			
		if (skb->end - skb->tail > len) {
			skb_put(skb, len);
			if (rtlpriv->use_new_trx_flow)	
				skb_reserve(skb, status.rx_drvinfo_size + status.rx_bufshift + 24);
			else
		...
		if (rtlpriv->use_new_trx_flow) {
			rtlpriv->mac80211.next_rx_read_point[hw_queue] = (rtlpriv->mac80211.next_rx_read_point[hw_queue]+1)%RTL_PCI_MAX_RX_COUNT;

			rx_remained_cnt--;
			if (1/*rx_remained_cnt == 0*/) {
				rtl_write_word(rtlpriv, 0x3B4/*REG_RXQ_TXBD_IDX*/, rtlpriv->mac80211.next_rx_read_point[hw_queue]);
			}
		}
		if (((rtlpriv->link_info.num_rx_inperiod +
			rtlpriv->link_info.num_tx_inperiod) > 8) ||
		...
		if (rtlpriv->use_new_trx_flow) {
			_rtl_pci_init_one_rxdesc(hw, (u8 *)buffer_desc, rxring_idx,
					rtlpci->rx_ring[rxring_idx].idx);			
		} else {
...
static irqreturn_t _rtl_pci_interrupt(int irq, void *dev_id)
{
	...
	rtl_write_dword(rtlpriv, 0xb0, 0x0);
	rtl_write_dword(rtlpriv, 0xb8, 0x0);
	...
	rtl_write_dword(rtlpriv, 0xb0, rtlpci->irq_mask[0]);
	rtl_write_dword(rtlpriv, 0xb8, rtlpci->irq_mask[1]);
...
/*<delete in kernel start>*/
	...
	rtlpriv->cfg->ops->fill_tx_desc(hw, hdr, (u8 *) pdesc, 
		(u8 *)pbuffer_desc, info, pskb, BEACON_QUEUE, &tcb_desc);
#else
/*<delete in kernel end>*/
	rtlpriv->cfg->ops->fill_tx_desc(hw, hdr, (u8 *) pdesc,
		(u8 *)pbuffer_desc, info, NULL, pskb, BEACON_QUEUE, &tcb_desc);
/*<delete in kernel start>*/
#endif
/*<delete in kernel end>*/
...
static void _rtl_pci_init_trx_var(struct ieee80211_hw *hw)
{
	...
	for (i = 0; i < RTL_PCI_MAX_TX_QUEUE_COUNT; i++)
		rtlpci->txringcount[i] = TX_DESC_NUM_92E;
...
static int _rtl_pci_init_rx_ring(struct ieee80211_hw *hw, int rxring_idx)
{
	...
	if (rtlpriv->use_new_trx_flow) {
		...
		/* alloc dma for this ring */
		rtlpci->rx_ring[rxring_idx].buffer_desc = pci_alloc_consistent(rtlpci->pdev,
		...
		for (i = 0; i < rtlpci->rxringcount; i++) {			
			...
			if (!_rtl_pci_init_one_rxdesc(hw, (u8 *)entry, rxring_idx, i))
				return -ENOMEM;
		}
	} else {
...
static void _rtl_pci_free_rx_ring(struct ieee80211_hw *hw,
			int rxring_idx)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_pci *rtlpci = rtl_pcidev(rtl_pcipriv(hw));
...
int rtl_pci_reset_trx_ring(struct ieee80211_hw *hw)
{
	...
	for (rxring_idx = 0; rxring_idx < RTL_PCI_MAX_RX_QUEUE;
		...
		if ((rtlpriv->use_new_trx_flow == false) && /*new trx flow, do nothing*/
			rtlpci->rx_ring[rxring_idx].desc) {
			struct rtl_rx_desc *entry = NULL;
...
/*<delete in kernel end>*/
	...
	if (hw_queue != BEACON_QUEUE) {
		if (rtlpriv->use_new_trx_flow)
			idx = rtlpriv->mac80211.current_tx_write_point[hw_queue];
		else
...
/*<delete in kernel start>*/
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3,7,0))
	rtlpriv->cfg->ops->fill_tx_desc(hw, hdr, (u8 *) pdesc, (u8 *)ptx_bd_desc,
