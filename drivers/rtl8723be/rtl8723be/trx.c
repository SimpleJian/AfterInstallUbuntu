...
#include "dm.h"
#include "phy.h"
u8 _rtl8723be_map_hwqueue_to_fwqueue(struct sk_buff *skb, u8 hw_queue)
{
...
static void _rtl8723be_query_rxphystatus(struct ieee80211_hw *hw,
			...
			struct rx_fwinfo_8723be *p_drvinfo, bool bpacket_match_bssid,
	...
	if (b_is_cck) {
		...
		if (bpacket_match_bssid) {
			for (i = RF90_PATH_A; i<= RF90_PATH_B; i++)
				rtl_priv(hw)->dm.cfo_tail[i] = (char)p_phystRpt->path_cfotail[i];
...
#if 0
static void _rtl8723be_smart_antenna(struct ieee80211_hw *hw,
	struct rtl_stats *pstatus)
{
	...
	(rtlefuse->antenna_div_type == CGCS_RX_HW_ANTDIV)) {
		if (pstatus->b_packet_toself || pstatus->b_packet_matchbssid) {
			antsel_tr_mux = (pfat_table->antsel_rx_keep_2 << 2) |
					(pfat_table->antsel_rx_keep_1 << 1) | pfat_table->antsel_rx_keep_0;
			rtl8723be_dm_ant_sel_statistics(hw, antsel_tr_mux, 0, pstatus->rx_pwdb_all);
		}
		...
		u8 *pdesc, struct rx_fwinfo_8723be *p_drvinfo)
{
	...
	if (ieee80211_is_beacon(fc))
		...
		b_packet_beacon = false;/* YP: it's not initialized, this assignment is neccesary, 
	...
	_rtl8723be_query_rxphystatus(hw, pstatus, pdesc, p_drvinfo,
				   ...
				   b_packet_beacon);
	/*_rtl8723be_smart_antenna(hw, pstatus); */
			   ...
			   u8 *pdesc, struct sk_buff *skb)
{
	...
	rx_status->rate_idx = _rtl8723be_rate_mapping(hw,
	...
	if (phystatus == true) {
		...
		_rtl8723be_translate_rx_signal_stuff(hw,
						   skb, status, pdesc,
						   p_drvinfo);
	}
			...
			if (ptcb_desc->empkt_num) {
				RT_TRACE(COMP_SEND, DBG_TRACE,
					 ("Insert 8 byte.pTcb->EMPktNum:%d\n",
					  ptcb_desc->empkt_num));
				_rtl8723be_insert_emcontent(ptcb_desc, (u8 *)(skb->data));
			}
		} else {
...
*/
	...
	/* rtl8723be_dm_set_tx_ant_by_tx_info(hw,pdesc,ptcb_desc->mac_id); */
	RT_TRACE(COMP_SEND, DBG_TRACE, ("\n"));
}

void rtl8723be_tx_fill_cmddesc(struct ieee80211_hw *hw,
...
void rtl8723be_set_desc(struct ieee80211_hw * hw, u8 *pdesc, bool istx, u8 desc_name, u8 *val)
{
	if (istx == true) {
		switch (desc_name) {
		case HW_DESC_OWN:
