...
void rtl8723be_dm_txpower_track_adjust(struct ieee80211_hw *hw,
...
static void rtl8723be_dm_init_dynamic_bb_powersaving(struct ieee80211_hw *hw)
{
...
static void rtl8723be_dm_init_dynamic_txpower(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
...
static void rtl8723be_dm_init_txpower_tracking(struct ieee80211_hw *hw)
{
	...
	rtlpriv->dm.bb_swing_idx_ofdm_base[0] = rtlpriv->dm.ofdm_index[0];
...
void rtl8723be_dm_init(struct ieee80211_hw *hw)
{
	...
	rtl8723be_dm_init_dynamic_bb_powersaving(hw);
	rtl8723be_dm_init_dynamic_txpower(hw);
...
static void rtl8723be_dm_dig(struct ieee80211_hw *hw)
{
	...
	bfirstconnect = (mac->link_state >= MAC80211_LINKED) && (dm_digtable.b_media_connect_0 == false);
	bfirstdisconnect = (mac->link_state < MAC80211_LINKED) && (dm_digtable.b_media_connect_0 == true);
	...
	if (mac->link_state >= MAC80211_LINKED) {
		if ((dm_digtable.rssi_val_min + 10) > dm_dig_max)
			...
			dm_digtable.rx_gain_range_max = dm_digtable.rssi_val_min + 10;
		...
		if(rtlpriv->dm.b_one_entry_only) {
			...
			else if (dm_digtable.rssi_val_min - offset > dig_maxofmin)
...
*/		}
	...
	else if (current_igi < dm_digtable.rx_gain_range_min)
		current_igi = dm_digtable.rx_gain_range_min;
...
static void rtl8723be_set_iqk_matrix(struct ieee80211_hw *hw,
	u8 ofdm_index,
	u8 rfpath,
	long iqk_result_x,
	long iqk_result_y)
	...
	u8 channel_mapped_index)
{
	...
	if (method == TXAGC) {
		...
		if (rfpath == RF90_PATH_A) {
			if (rtldm->bb_swing_idx_ofdm[RF90_PATH_A] < bb_swing_idx_ofdm_limit)
				bb_swing_idx_ofdm_limit = rtldm->bb_swing_idx_ofdm[RF90_PATH_A];

			rtl8723be_set_iqk_matrix(hw, rtldm->bb_swing_idx_ofdm[rfpath], rfpath,
			...
			if (rtldm->bb_swing_idx_ofdm[RF90_PATH_B] < bb_swing_idx_ofdm_limit)
				bb_swing_idx_ofdm_limit = rtldm->bb_swing_idx_ofdm[RF90_PATH_B];
...
static void rtl8723be_dm_txpower_tracking_callback_thermalmeter(struct ieee80211_hw
							     *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	...
	char delta_swing_table_idx_tup_a[] =   {0, 0, 1, 2, 2, 2, 3, 3, 3, 4,  5,  5,  6,  6,  7,  7,  8,  8,  9,  9,  9, 10, 10, 11, 11, 12, 12, 13, 14, 15};
	char delta_swing_table_idx_tdown_a[] = {0, 0, 1, 2, 2, 2, 3, 3, 3, 4,  5,  5,  6,  6,  6,  6,  7,  7,  7,  8,  8,  9,  9, 10, 10, 11, 12, 13, 14, 15};
	...
	if (delta > 0 && rtlpriv->dm.txpower_track_control) {
		...
		if (rtldm->delta_power_index[RF90_PATH_A] == rtldm->delta_power_index_last[RF90_PATH_A])
			rtldm->power_index_offset[RF90_PATH_A] = 0;
		else
			rtldm->power_index_offset[RF90_PATH_A] = rtldm->delta_power_index[RF90_PATH_A] - rtldm->delta_power_index_last[RF90_PATH_A];

		rtldm->ofdm_index[0] = rtldm->bb_swing_idx_ofdm_base[RF90_PATH_A] + rtldm->power_index_offset[RF90_PATH_A];
		rtldm->cck_index = rtldm->bb_swing_idx_cck_base + rtldm->power_index_offset[RF90_PATH_A];
		
		rtldm->bb_swing_idx_cck = rtldm->cck_index;
		rtldm->bb_swing_idx_ofdm[0] = rtldm->ofdm_index[0];
	...
	if ((rtldm->power_index_offset[RF90_PATH_A] != 0) && 
		...
		if (thermalvalue > rtlefuse->eeprom_thermalmeter)
			rtl8723be_dm_tx_power_track_set_power(hw, BBSWING, 0, index_for_channel);
		else
			rtl8723be_dm_tx_power_track_set_power(hw, BBSWING, 0, index_for_channel);

		rtldm->bb_swing_idx_cck_base = rtldm->bb_swing_idx_cck;
		rtldm->bb_swing_idx_ofdm_base[RF90_PATH_A] = rtldm->bb_swing_idx_ofdm[0];
...
static void rtl8723be_dm_check_edca_turbo(struct ieee80211_hw *hw)
{
	...
	b_bias_on_rx = (iot_peer == PEER_RAL || iot_peer == PEER_ATH)?true:false;
	b_edca_turbo_on = ((!rtlpriv->dm.bis_any_nonbepkts)&&(!rtlpriv->dm.b_disable_framebursting))?true:false;
	...
	{
		if (b_bias_on_rx)
			b_is_cur_rdlstate = (cur_txok_cnt > cur_rxok_cnt*4)?false:true;	
		else
			b_is_cur_rdlstate = (cur_rxok_cnt > cur_txok_cnt*4)?true:false;
...
static void rtl8723be_dm_cck_packet_detection_thresh(struct ieee80211_hw *hw)
{
	...
	if (rtlpriv->mac80211.link_state >= MAC80211_LINKED) {
		/*dm_digtable.rssi_val_min = rtl8723be_dm_initial_gain_min_pwdb(hw);*/
...
void rtl8723be_dm_dynamic_edcca(struct ieee80211_hw *hw)
{
	...
	rtlpriv->cfg->ops->get_hw_reg(hw,HW_VAR_FW_PSMODE_STATUS,(u8*)(&b_fw_current_in_ps_mode));
...
void rtl8723be_dm_dynamic_atc_switch(struct ieee80211_hw *hw)
{
	...
	if (rtlpriv->mac80211.link_state < MAC80211_LINKED){
		if (rtldm->atc_status == ATC_STATUS_OFF) {
			rtl_set_bbreg(hw,ROFDM1_CFOTRACKING,BIT(11),ATC_STATUS_ON);
			rtldm->atc_status = ATC_STATUS_ON;
		}
		if (rtlpriv->cfg->ops->get_btc_status()) {
			if (!rtlpriv->btcoexist.btc_ops->btc_is_bt_disabled(rtlpriv)){
				RT_TRACE(COMP_BT_COEXIST, DBG_LOUD, ("odm_DynamicATCSwitch(): Disable CFO tracking for BT!!\n"));
				return;
			}
		}
		...
		if (cfo_ave < CFO_THRESHOLD_ATC && cfo_ave > -CFO_THRESHOLD_ATC){
			if (rtldm->atc_status == ATC_STATUS_ON){
				rtl_set_bbreg(hw,ROFDM1_CFOTRACKING,BIT(11),ATC_STATUS_OFF);
