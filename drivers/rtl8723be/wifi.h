...
struct rtl_mac {
	...
	/* add for 8192EE */
	u16 available_desc_to_write[MAX_TX_QUEUE];
	u16 current_tx_write_point[MAX_TX_QUEUE];
	u16 current_tx_read_point[MAX_TX_QUEUE];
	u16 next_rx_read_point[MAX_TX_QUEUE];
	u16 num_tx_desc[MAX_TX_QUEUE];
};
...
struct rtl_efuse {
	...
	u8 txpwrlevel_ht40_1s[MAX_RF_PATH][CHANNEL_MAX_NUMBER];	/*For HT 40MHZ pwr */
	u8 txpwrlevel_ht40_2s[MAX_RF_PATH][CHANNEL_MAX_NUMBER];	/*For HT 40MHZ pwr */
	char txpwr_cckdiff[MAX_RF_PATH][MAX_TX_COUNT]; /*CCK_24G_Diff*/
	char txpwr_ht20diff[MAX_RF_PATH][MAX_TX_COUNT];	/*BW20_24G_Diff*//*HT 20<->40 Pwr diff */
	char txpwr_ht40diff[MAX_RF_PATH][MAX_TX_COUNT];/*BW40_24G_Diff*/
	char txpwr_legacyhtdiff[MAX_RF_PATH][MAX_TX_COUNT];/*OFDM_24G_Diff*/	/*For HT<->legacy pwr diff */
...
struct rtl_hal_ops {
	...
	void (*pre_fill_tx_bd_desc) (struct ieee80211_hw *hw, 
									u8 *tx_bd_desc,
									u8 *desc, u8 queue_index, 
									struct sk_buff *skb, dma_addr_t addr);
	u16 (*rx_desc_buff_remained_cnt) (struct ieee80211_hw *hw,u8 queue_index);
	void (*rx_check_dma_ok) (struct ieee80211_hw *hw, u8 *header_desc, u8 queue_index);
	void (*fill_tx_desc) (struct ieee80211_hw * hw,
			      struct ieee80211_hdr * hdr, u8 * pdesc_tx, u8 * pbd_desc,
...
/*<delete in kernel end>*/
	...
	void (*set_desc) (struct ieee80211_hw *hw, u8 * pdesc, bool istx, u8 desc_name, u8 * val);
	 u32(*get_desc) (u8 * pdesc, bool istx, u8 desc_name);
	bool (*is_tx_desc_closed) (struct ieee80211_hw *hw, u8 hw_queue, u16 index); 
	...
	void (*fill_h2c_cmd) (struct ieee80211_hw *hw, u8 element_id, u32 cmd_len, u8 *p_cmdbuffer);
...
struct rtl_btc_ops {
	...
	void (*btc_mediastatus_notify) (struct rtl_priv *rtlpriv, enum rt_media_status mstatus);
	void (*btc_periodical) (struct rtl_priv *rtlpriv);
	void (*btc_halt_notify) (void);
	void (*btc_btinfo_notify) (struct rtl_priv *rtlpriv, u8 * tmp_buf, u8 length);
